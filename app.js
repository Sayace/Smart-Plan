  ...
  if(req.user===undefined){
    ...
    if (Object.keys(req.query).length === 0) {
      ...
      for (const prop in filterBy) {
        keysVal.push(prop);
        keysVal.push(filterBy[prop]);
      }
      actualKeyRank = keysVal.filter(element => (element === "rank"));
      actualKeySemester = keysVal.filter(element => (element === "semester"));
      actualValue = keysVal.filter(element => (element !== "rank" && element !== "semester"));

      let key = null;
      if(actualKeyRank.length === 0){
        key = actualKeySemester[0];
      }
      else{
        key = actualKeyRank[0];
      }
      ...
      if(key === "rank"){
        filtered = {user: res.locals.user._id, rank:val};
      }
      else if(key === "semester"){
        filtered = {user: res.locals.user._id, semester:val};
      }
